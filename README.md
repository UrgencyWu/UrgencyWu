# 👋 Hi, I'm Shaw Wu | 吴同学

---

### 🎓 关于我 (About Me)

- **🏫 教育背景**：
  - 哈尔滨工业大学 2025～now 软件工程硕士在读，focus on LLM application on SysML
  - 中国农业大学 2024～2025 数据科学与大数据技术 第二学士
  - 东华大学 2020～2024 旅游管理 学士
  - 期间实习：Daxue Consulting 项目助理、Volvo 供应链专员

- **💡 转型之路**：从商科"写方案"思维转向"做工程"落地——CS 其实是我最开始就想学的专业

- **🛠️ 当前目标**：专注于与生活息息相关的项目，输出以人为导向的应用和工具

- **🏸 业余爱好**：羽毛球/摄影/音乐/背包旅行

---

### 🛠️ 技能树 (Tech Stack)

- Python / PyTorch — 主力工具，还在熟练的过程中
- 大数据 — Spark、Hadoop，了解但不算精通
- LLM 应用 — RAG、Agent 方向，最近的重点

---

### 📬 联系我 (Contact Me)

- **Email**: [shaw-wu@qq.com](mailto:shaw-wu@qq.com)
- **GitHub**: [@UrgencyWu](https://github.com/UrgencyWu)
